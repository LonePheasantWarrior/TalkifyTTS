...
9. [已实现功能](#已实现功能)
10. [日志与错误处理](#日志与错误处理)
11. [语音预览功能](#11-语音预览功能demo-service)
12. [同步处理机制与资源管理](#11-同步处理机制与资源管理)
13. [架构优化与近期修复](#12-架构优化与近期修复)
14. [开发规范补充](#13-开发规范补充)
15. [启动网络检查功能](#14-启动网络检查功能)
16. [错误处理改进](#15-错误处理改进)
17. [豆包语音合成2.0引擎实现](#17-豆包语音合成20引擎实现)
...
| 依赖 | 用途 |
|------|------|
| DashScope SDK | 阿里云百炼通义千问3语音合成 |
| OkHttp | 火山引擎豆包语音合成2.0 HTTP 流式 API |
...
│   │   ├── BaseEngineConfig.kt        # 引擎配置基类（抽象类，voiceId 为唯一通用属性）
│   │   ├── Qwen3TtsConfig.kt          # 通义千问3引擎配置（继承 BaseEngineConfig）
...
| `BaseEngineConfig` | 抽象类 | 引擎配置基类，voiceId 为唯一通用属性 |
| `Qwen3TtsConfig` | 数据类 | 通义千问3引擎配置，继承 BaseEngineConfig（apiKey, voiceId） |
| `SeedTts2Config` | 数据类 | 豆包语音合成2.0引擎配置，继承 BaseEngineConfig（apiKey, voiceId） |
...
| `EngineConfigRepository` | 接口 | 引擎配置存取标准方法（返回 BaseEngineConfig） |
| `AppConfigRepository` | 接口 | 全局配置存取标准方法 |

**配置架构设计原则**：
- 引擎配置采用层次化设计：`BaseEngineConfig` 为抽象基类
- 基类只定义通用属性 `voiceId`，具体引擎配置子类定义各自的特有属性
- 示例：`Qwen3TtsConfig` 在基类基础上增加 `apiKey` 属性
- 仓储接口统一返回 `BaseEngineConfig`，具体类型由运行时决定
- UI 层通过 `when` 表达式或引擎工厂方法处理不同配置类型
...
**engine/ 目录 - 引擎特定实现**：
- `Qwen3TtsVoiceRepository`：从资源文件加载通义千问3声音列表
- `Qwen3TtsConfigRepository`：使用 SharedPreferences 持久化通义千问3引擎配置
  - 按引擎 ID 隔离存储不同引擎的配置
  - 存储位置：`talkify_engine_configs.xml`
- `SeedTts2VoiceRepository`：从资源文件加载豆包语音合成2.0声音列表
- `SeedTts2ConfigRepository`：使用 SharedPreferences 持久化豆包语音合成2.0引擎配置
  - 按引擎 ID 隔离存储不同引擎的配置
  - 存储位置：`talkify_engine_configs.xml`
...
- 委托引擎执行实际的语音合成
- 完整的异常处理和错误通知机制
- **详细功能说明**：同步处理机制、资源管理、优雅关闭请参阅[第 11 章](#11-同步处理机制与资源管理)
...
- `TtsEngineApi`：引擎抽象接口，定义引擎必须实现的核心方法
  ...
  - `createDefaultConfig()`：创建引擎默认配置实例（用于 UI 层动态创建正确的配置类型）
  - `getConfigLabel()`：获取配置项的本地化标签（解耦 UI 与引擎特定标签）
...
- `AudioConfig.SEED_TTS2`：豆包语音合成2.0默认配置（24000Hz、PCM_16BIT、单声道）
...
- `TtsEngineFactory`：引擎工厂，根据引擎 ID 创建对应的引擎实例
  ...
  - **已注册引擎**：`qwen3-tts`（通义千问3）、`seed-tts-2.0`（豆包语音合成2.0）

**service/engine/impl/ 目录 - 引擎具体实现**：
- `Qwen3TtsEngine`：通义千问3引擎实现
  - 引擎 ID：`qwen3-tts`
  - 支持语言：zho, eng, deu, ita, por, spa, jpn, kor, fra, rus（10种）
  - **静态常量**：`SUPPORTED_LANGUAGES` 数组，无需创建实例即可获取支持语言
- `SeedTts2Engine`：豆包语音合成2.0引擎实现
  - 引擎 ID：`seed-tts-2.0`
  - 基于 OkHttp 实现 HTTP 流式 API 调用
  - 支持连接复用（ConnectionPool，keep-alive 1分钟）
  - 支持语言：zho, eng（2种）
  - **静态常量**：`SUPPORTED_LANGUAGES` 数组，无需创建实例即可获取支持语言
...
| 引擎配置 | `EngineConfigRepository` | 引擎特定配置（BaseEngineConfig 子类） | `talkify_engine_configs.xml` |
...
不同 TTS 引擎的配置项各不相同：
- 通义千问3：`apiKey` + `voiceId`
- 豆包语音合成2.0：`apiKey` + `voiceId`
- 其他引擎可能需要：`secretKey`、`modelId`、`endpoint` 等
...
interface EngineConfigRepository {
    fun getConfig(engineId: String): BaseEngineConfig  // 返回基类类型
...
每个引擎必须实现以下工厂方法，由 UI 层调用：

**createDefaultConfig()** - 创建默认配置实例：
...
### ConfigBottomSheet 集成

配置底部弹窗使用引擎工厂方法实现完全解耦：
...
fun ConfigBottomSheet(
    ...
    val engine = TtsEngineFactory.createEngine(currentEngine.id)
    val defaultConfig = engine.createDefaultConfig()

    val getLabel: (String) -> String? = { key ->
        engine.getConfigLabel(key, context)
    }

    val configItems = buildConfigItems(config, getLabel)
    // ...
}
```
...
1. **创建配置类**：继承 `BaseEngineConfig`，添加引擎特有属性
2. **实现工厂方法**：在引擎类中实现 `createDefaultConfig()` 和 `getConfigLabel()`
3. **更新仓储**：确保 `EngineConfigRepository` 能正确序列化/反序列化新配置类型
4. **更新 UI**：在 `ConfigBottomSheet` 的 `buildConfigItems()` 和 `buildConfigFromItems()` 中添加 `when` 分支
...
- `app/src/main/res/values/volcengine-Seed-TTS-2-voices.xml`：火山引擎豆包语音合成2.0的声音配置
  - voice_ids：声音标识
  - display_names：中文显示名称
  - descriptions：声音特色描述
  - supported_languages：支持语言
...
- 当前支持引擎：阿里云百炼"通义千问3语音合成"、火山引擎"豆包语音合成2.0"
...
**新增引擎步骤**：
1. **创建配置类**：继承 `BaseEngineConfig`，定义引擎特有属性
2. **实现引擎类**：继承 `AbstractTtsEngine`，实现 `TtsEngineApi` 接口
   - 在伴生对象中定义 `ENGINE_ID`、`ENGINE_NAME`、`SUPPORTED_LANGUAGES`
   - 实现 `synthesize()` 方法完成语音合成
   - 实现 `getSupportedLanguages()` 返回 `SUPPORTED_LANGUAGES.toSet()`
   - 实现 `createDefaultConfig()` 返回默认配置实例
   - 实现 `getConfigLabel()` 返回配置项本地化标签
3. **创建 Repository**：实现 `VoiceRepository` 和 `EngineConfigRepository`
4. **注册引擎**：在 `TtsEngineFactory.initializeEngines()` 中注册引擎
5. **添加资源文件**：在 `res/values/` 中添加引擎声音配置文件
6. **更新 UI**：在 `ConfigBottomSheet` 中添加配置项处理逻辑
7. **更新 `TalkifyCheckDataActivity`**：在语言列表获取中添加新引擎分支

**示例：豆包语音合成2.0 引擎结构**
```
domain/model/
├── SeedTts2Config.kt                    # 引擎配置数据类
infrastructure/engine/repo/
├── SeedTts2VoiceRepository.kt           # 声音仓储实现
├── SeedTts2ConfigRepository.kt          # 配置仓储实现
service/engine/impl/
├── SeedTts2Engine.kt                    # 引擎实现类
    ├── companion object
    │   ├── ENGINE_ID = "seed-tts-2.0"
    │   ├── ENGINE_NAME = "豆包语音合成2.0"
    │   └── SUPPORTED_LANGUAGES = arrayOf("zho", "eng")
...
- `TalkifyCheckDataActivity`：TTS 数据检查 Activity
  - 用于系统 TTS 集成时的数据检查
  - 接收系统 TTS 引擎的回调请求
- `TalkifyDownloadVoiceData`：语音数据下载 Activity
  - 用于下载和管理语音数据资源
...
- 系统通知通道 importance 为 `IMPORTANCE_HIGH`，支持 heads-up 悬浮通知
...
- `TalkifyNotificationHelper` 快捷通知发送 Helper
  - `sendTtsPlaybackNotification()`：发送"Talkify 正在工作"通知
...
**设计原则**：
根据 Android 官方文档，`onSynthesizeText` 方法应阻塞直到合成完成。TalkifyTtsService 采用同步处理模式，直接在当前线程阻塞等待合成结果，符合 Android TTS Service 标准生命周期。

**同步处理流程**：
- `onSynthesizeText()` 调用 `processRequestSynchronously()` 直接处理
- 使用 `CountDownLatch` 实现线程同步，等待合成完成
- 设置 120 秒超时防止网络永久挂起
- 超时或错误时自动终止引擎请求并返回错误

**资源管理**：
- **WakeLock**：防止合成过程中设备休眠，设置部分唤醒锁，最长持有 10 分钟
- **WifiLock**：确保合成期间 WiFi 保持高性能模式，防止网络中断
- **前台服务**：提升服务优先级，防止被系统查杀
...
**工作流程**：
```
用户发起请求 → onSynthesizeText()
    ↓
processRequestSynchronously() 同步处理
    ↓
获取 WakeLock + WifiLock + 启动前台服务
    ↓
初始化音频参数并调用 callback.start()
    ↓
设置 CountDownLatch 门闩
    ↓
引擎执行合成，流式回调音频数据
    ↓
阻塞等待门闩（超时 120 秒）
    ↓
完成/错误 → callback.done()/callback.error()
    ↓
清理资源：释放锁 + 停止前台服务
```
...
豆包语音合成2.0是火山引擎提供的云端 TTS 服务，基于大模型技术实现高质量语音合成。
...
**HTTP 流式 API**：
- 使用 OkHttp 实现 HTTP 单向流式接口
- 请求地址：`https://openspeech.bytedance.com/api/v3/tts/unidirectional`
- 认证方式：Header 中携带 `x-api-key`
- 连接复用：使用 ConnectionPool 实现 TCP 连接复用（keep-alive 1分钟）

**请求参数**：
```json
{
  "user": {"uid": "talkify_user_xxx"},
  "req_params": {
    "text": "要合成的文本",
...
**响应处理**：
- 流式返回 JSON 行（line-delimited JSON）
- 音频数据以 Base64 编码返回
- 使用 `code == 20000000` 判断合成完成
- 使用 `X-Tt-Logid` 头进行问题追踪

#### 17.3 核心组件

**SeedTts2Config** (`domain/model/SeedTts2Config.kt`)：
```kotlin
data class SeedTts2Config(
    override val voiceId: String = "",
    val apiKey: String = ""
...
**SeedTts2Engine** (`service/engine/impl/SeedTts2Engine.kt`)：
...
关键方法：
- `synthesize()`：执行语音合成，使用协程顺序处理文本块
- `buildHttpRequest()`：构建 HTTP 请求，使用 AudioConfig 配置音频参数
- `processStreamResponse()`：处理流式响应，解析 Base64 音频数据
- `convertSpeechRate()` / `convertLoudnessRate()`：参数值转换
...
private fun parseVolcEngineError(errorBody: String): String {
    return when (code) {
        45000030 -> "资源未授权：请在火山引擎控制台开通服务"
        45000001 -> "认证失败：请检查 API Key 是否正确"
        45000002 -> "参数错误"
        45000003 -> "请求过于频繁"
        45000004 -> "服务暂时不可用"
        45000005 -> "余额不足"
...
**资源文件** (`res/values/volcengine-Seed-TTS-2-voices.xml`)：

包含 16 种声音：
- Vivi 2.0、小何 2.0、云舟 2.0、小天 2.0（通用场景）
- 儿童绘本、大壹、黑猫侦探社咪、鸡汤女（有声阅读/视频配音）
- 魅力女友、流畅女声、儒雅逸辰（视频配音）
- 可爱女生、调皮公主、爽朗少年、天才同桌、知性灿灿（角色扮演）
...
res/values/
├── volcengine-Seed-TTS-2-voices.xml               # 声音配置资源
```

#### 17.8 设计决策

**为什么选择 OkHttp 而不是专用 SDK？**
- 火山引擎官方 SDK 较为重量级
- HTTP API 文档完善，直接使用 OkHttp 更轻量
- 便于控制连接池和超时参数
- 统一的错误处理流程

**为什么使用静态常量 SUPPORTED_LANGUAGES？**
- `TalkifyCheckDataActivity` 需要在不创建引擎实例的情况下获取支持语言
- 避免不必要的对象创建和初始化开销
- 便于其他组件（如设置页面）快速获取引擎信息

**为什么音频格式使用 PCM？**
- Android TTS 框架原生支持 PCM 格式
- 无需在客户端进行音频解码
- 减少 CPU 开销，降低延迟
