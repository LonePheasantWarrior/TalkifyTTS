...
- **本地TTS引擎**: sherpa-onnx 1.12.12 (离线语音合成引擎，支持 VITS 和 Kokoro 模型)
...
│   │   ├── LocalModelInfo.kt              // 本地模型元数据（modelType, downloadUrl, voices, 采样率, 支持语言）
...
│   │   │   └── TtsTelemetryTracker.kt    // 语音合成事件追踪（限频上报 providerId_modelId 维度）
│   │   ├── update/                       // 更新检查
...
│   │       ├── LocalModelManager.kt       // 模型生命周期管理（部署状态、下载状态、文件路径）
...
│   │       └── LocalTTSProvider.kt        // 本地模型离线合成供应商（sherpa-onnx，引擎空闲自动释放）
└── ui/                                   // 【表现层】Jetpack Compose UI
    ├── components/                       // 通用 UI 组件
    │   ├── BatteryOptimizationDialog.kt
    │   ├── ConfigBottomSheet.kt          // 配置弹窗（含本地模型选择、下载状态显示、下载触发）
    │   ├── ConfigEditor.kt
    │   ├── ModelManagementDialog.kt      // 本地模型管理对话框（模型列表、下载/删除、状态同步）
    │   ├── ProviderSelector.kt
    │   ├── MarkdownText.kt
    ...
    │   ├── demo/DemoPlaybackController.kt // Demo 试听状态与播放控制
    │   └── model/LocalModelDownloadController.kt // 下载进度、广播接收、冲突检测
...
- **`AbstractTtsProvider`**: 提供通用实现（日志、释放状态检查、可朗读文本检测、元数据方法下沉）。音色/语言类元数据基于钩子属性实现：
  ...
  - 基类据此统一实现 `getSupportedVoices()` / `getDefaultVoiceId()` / `isVoiceIdCorrect()` / `getSupportedLanguages()` / `getDefaultLanguages()` / `getConfigLabel()`
...
- **`AbstractTtsProvider` 之外的特例**：`LocalTTSProvider` 直接实现 `TtsProviderApi`（本地引擎合成流程与配置模型完全不同）；`AliyunBailianProvider` 使用 DashScope SDK 独立实现串行分块（SDK 全局 `Constants.baseHttpApiUrl` 静态端点限制，代码中有注释说明）；`TencentCloudProvider` 受官方 SDK 会话模型限制保持串行分块（单块 30s 超时保护）。
...
│  ModelManagementDialog (模型管理)       │
│  MainViewModel (下载状态管理)           │
├──────────────────────────────────────┤
│  Service Layer                       │
│  LocalTTSProvider (TtsProviderApi)   │
├──────────────────────────────────────┤
│  Infrastructure Layer                │
│  ┌────────────────────────────────┐  │
│  │ SherpaTtsEngine               │  │
│  │ (OfflineTts 封装, VITS/Kokoro)│  │
│  ├────────────────────────────────┤  │
│  │ LocalModelManager             │  │
│  │ (模型生命周期: 部署/下载/状态)   │  │
│  ├────────────────────────────────┤  │
...
`LocalModelRegistry` 是所有本地模型的**单一数据源**，定义了每个模型的完整元信息。当前注册了三款模型：

| 模型 ID | 引擎类型 | 模型大小 | 采样率 | 支持语言 | 文件组成 |
|---------|---------|---------|--------|---------|---------|
| `vits-zh-aishell3` | VITS | ~45 MB | 22050 Hz | 中文普通话 | model.onnx + tokens.txt + lexicon.txt |
| `vits-cantonese-xiaomai` | VITS | ~50 MB | 22050 Hz | 粤语 | model.onnx + tokens.txt + lexicon.txt |
| `kokoro-82m` | Kokoro | ~440 MB | 24000 Hz | 中/英/日/法/韩… (多语言) | model.onnx + tokens.txt + voices.bin + lexicons + fst 文件 + espeak-ng-data.tar.bz2 |

每个模型定义了：
- **modelType**: `VITS` 或 `KOKORO`，决定 SherpaTtsEngine 使用不同的初始化参数
- **downloadFileInfo**: `Map<URL, FileName>`，完整下载 URL → 本地文件名的映射；默认 URL 使用 hf-mirror.com 国内镜像
- **archiveAssets**: `Map<URL, Dir>`（Kokoro 特有），需要下载并解压的归档文件（如 espeak-ng 语音数据）
- **voiceList**: `List<LocalModelVoice>`，模型支持的语音列表（voiceId, displayName, language）
- **sampleRate**: 模型的原生采样率
- **构造 fallback URL**: `getFallbackUrls()` 将默认 hf-mirror 源替换为 huggingface.co 原始源

##### 模型文件管理（LocalModelManager）

`LocalModelManager` 负责模型的本地文件生命周期：
- **内存缓存（DCL 懒加载）**: `LocalTTSProvider` 持有全局唯一实例，通过 DCL 双重检查锁定确保多线程安全
- **文件路径**: 模型文件存储在 `context.getExternalFilesDir("models")/{modelId}/` 下
- **部署状态**: 检查模型所需的所有文件是否存在来判断 `isDeployed()`
- **下载状态追踪**: `isDownloading()` 检查是否有正在进行的下载任务

##### SherpaTtsEngine 引擎封装

`SherpaTtsEngine` 是对 sherpa-onnx `OfflineTts` 的 Kotlin 封装：
- **初始化**: 根据 `modelType`（VITS/Kokoro）使用不同的 `OfflineTtsConfig` 构建参数
  - VITS: `modelConfig.vits.model` + `modelConfig.tokens`
  - Kokoro: 额外需要 `modelConfig.kokoro.model` + `modelConfig.kokoro.voices` + `modelConfig.kokoro.dataDir`（espeak-ng 语音数据目录）
- **合成方法 `synthesize(text, sid, speed)`**: 传入文本、说话人 ID 和语速，返回 `OfflineTtsGeneratedAudio`（包含 PCM samples 数组和采样率）
...
- **下载策略**:
  ...
  3. 逐个下载文件，通过 OkHttp 请求，实时计算总进度
  4. 失败时通过 `getFallbackUrls()` 切换至备用源（huggingface.co）重试
  5. 对于 Kokoro 模型，额外下载 `archiveAssets` 中的归档文件并解压
...
enum class LocalModelType {
    VITS,     // VITS 模型（中文/粤语），使用 vits model config
    KOKORO    // Kokoro 多语言模型，使用 kokoro model config（需 espeak-ng 数据）
}
...
TalkifyTtsService
    ...
    ▼
TtsProviderFactory.createManufacturer()  →  LocalTTSProvider
    │
    │  1. 检查模型是否已下载
    │     ├─ 已部署 → 直接合成
    │     └─ 未下载 → 下载模型（LocalModelDownloadService）
    │                    │
    │                    ├─ 下载中 → 等待下载完成
    │                    ├─ 成功 → 部署模型 → 进入合成
    │                    └─ 失败 → 抛出异常
    ...
    │     ├─ 调用 engine.synthesize(text, sid, speed)
    │     ├─ 获取 PCM samples (FloatArray) + sampleRate
    │     ├─ 分块输出（chunkSize = sampleRate * 0.5, 即 0.5 秒片段）
    │     └─ 通过 flow.emit() 流式发送 SynthesisResult.Message
    │
    ▼
AudioTrack 播放
```
...
**LocalTTSProvider** 直接实现 `TtsProviderApi`（不继承 `AbstractTtsProvider`），支持以下合成参数：
...
由于 sherpa-onnx 的离线引擎是**全量合成**（输入完整文本，一次性返回全部音频），LocalTTSProvider 通过以下方式模拟流式效果：

```kotlin
// 分块输出策略
val chunkSize = sampleRate / 2  // 每块 0.5 秒音频
var offset = 0
while (offset < samples.size) {
    val end = minOf(offset + chunkSize, samples.size)
    val chunk = samples.copyOfRange(offset, end)
    emit(SynthesisResult.Message(samplesToPcm16(chunk)))
    offset = end
}
```
...
`AudioConfig.LocalTTS` 常量默认使用 24000 Hz，实际合成时会根据模型覆盖采样率。
...
下载完成后，`LocalModelManager.isDeployed()` 检查所有必需文件是否存在。

---

## 5. 配置存储方案

应用采用轻量级的 `SharedPreferences` 进行配置持久化。供应商配置统一由 `BasePrefsConfigRepository<T>` 泛型基类承载——所有供应商共用同一个 SharedPreferences 文件（`talkify_engine_configs`），以 `engine_{providerId}_` 键前缀隔离；子类只需声明 `serialize`/`deserialize` 两个键值映射函数（兼容旧版本原生类型写入的数据），`hasConfig` 统一为"前缀下任一非空值"。
...
3.  **本地模型配置 (`LocalModelConfigRepository`)**
    - 存储：独立的 SharedPreferences 文件 (`talkify_local_model_config`)
...
4.  **本地模型音色 (`LocalModelVoiceRepository`)**
    - 不从 XML 文件读取，直接查询 `LocalModelRegistry.getById(modelId)?.voices`
    - 音色列表随模型 ID 变化而动态更新

5.  **模型文件存储**
    - 目录：`context.getExternalFilesDir("models")/{modelId}/`
    - 内容：引擎文件（`model.onnx`, `tokens.txt`）+ Kokoro 额外数据（`espeak-ng-data/` 目录）
    - 生命周期：文件由 `LocalModelDownloadService` 下载，`LocalModelManager.isDeployed()` 检查完整性
    - 清理：用户可通过模型管理对话框删除模型文件，释放存储空间
...
1. **定义模型信息**：在 `LocalModelRegistry.kt` 中新增一个 `LocalModelInfo` 条目：
   ...
   LocalModelInfo(
       ...
       downloadUrl = mapOf(
           DownloadSource.HF_MIRROR to "https://hf-mirror.com/...",
           DownloadSource.HUGGINGFACE to "https://huggingface.co/..."
       ),
       files = listOf("model.onnx", "tokens.txt"),  // 所需文件列表
       voices = listOf(
           VoiceItem("speaker_0", "说话人0"),
           VoiceItem("speaker_1", "说话人1")
       ),
       sampleRate = 22050,
...
> - **本地模型（localModel）**：离线语音合成供应商，无需 API Key，引擎为 sherpa-onnx，Provider 实现为 `LocalTTSProvider`，模型定义在 `LocalModelRegistry`，音色列表由模型注册表动态提供。支持 VITS（中文/粤语）和 Kokoro（多语言）两种引擎类型。
