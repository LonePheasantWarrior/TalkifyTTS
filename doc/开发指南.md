...
- **本地TTS引擎**: sherpa-onnx v1.13.1 (离线语音合成引擎，支持 VITS 和 Kokoro 模型)
...
- **sampleRate**: 模型的原生采样率
- **备用下载源**: 常量 `DEFAULT_HF_MIRROR`（hf-mirror.com 国内镜像）与 `FALLBACK_HF_ORIGIN`（huggingface.co 原始源）；下载失败时将 URL 中的镜像域名替换为原始域名重试
...
- **下载策略**:
  ...
  4. 失败时将 URL 中的 hf-mirror.com 域名替换为 huggingface.co 原始源重试
