...
│   │       └── LocalModelVoiceRepository.kt  // 本地模型音色仓储（从 Registry 读取）
│   │   └── local/                         // 本地模型基础设施
...
│   │       ├── MiniMaxProvider.kt
│   │       ├── XiaomiProvider.kt
│   │       └── LocalTTSProvider.kt        // 本地模型离线合成供应商（sherpa-onnx，自动下载模型）
...
- **`AbstractTtsProvider`**: 提供通用实现（日志、释放状态检查、可朗读文本检测、通用配置项标签 `api_url`/`model_id`/`voice_id` 的默认实现）。**注意**：`LocalTTSProvider` 不继承此类，而是直接实现 `TtsProviderApi`，因其合成流程（本地引擎 vs HTTP/SSE）和配置模型完全不同。
