...
- 提取合成参数：pitch、speechRate、volume 封装为 `SynthesisParams`
- 完善的空指针检查和异常处理
...
- `TtsEngineApi`：引擎抽象接口，定义引擎必须实现的核心方法
  ...
  - `stop()`：停止合成
  - `release()`：释放资源
  - `SynthesisParams`：合成参数数据类（pitch、speechRate、volume）
...
**工作流程**：
```
onSynthesizeText() 被调用
    ↓
请求被放入 requestQueue (LinkedBlockingQueue)
    ↓
函数立即返回
    ↓
后台处理器循环：
